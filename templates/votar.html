...
async function cargarCandidatosPorMunicipio() {
  ...
  // Destruir select2 si estaba activo
  ...
  // Tomar textos reales seleccionados
  const dep = $('#departamento').val();
  const prov = $('#provincia').val();
  const munNombre = $('#municipio option:selected').text(); // IMPORTANTE: nombre, no ID

  // Si aún no hay municipio seleccionado (o quedó en placeholder)
  if (!munNombre || munNombre.toLowerCase().includes("selecciona")) {
  ...
  try {
    const url =
      `/api/candidatos?departamento=${encodeURIComponent(dep)}&provincia=${encodeURIComponent(prov)}&municipio=${encodeURIComponent(munNombre)}`;
